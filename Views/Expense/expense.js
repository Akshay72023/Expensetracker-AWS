...
function ShowPerPage(){
  ...
  localStorage.setItem('ItemPerPage',itemPerPage);
    ...
    async function onSubmit(e){
        ...
        const token= localStorage.getItem('token');
        try{
            const response= await axios.post('http://16.171.60.25:3000/expense/addexpense',obj,{headers:{'Authorization':token}});
    ...
    function parseJwt (token) {
      ...
      window.addEventListener('DOMContentLoaded' ,async ()=>{  
        ...
        try {
            const token= localStorage.getItem('token');
            const response = await axios.get(`http://16.171.60.25:3000/expense/getallexpense?page=${page}&itemPerPage=${ItemPerPage}`, { headers: {"Authorization" : token}});
      ...
      function getExpenses(page) {
        const token = localStorage.getItem('token');
        axios
          .get(`http://16.171.60.25:3000/expense/getallexpense?page=${page}&itemPerPage=${ItemPerPage}`, { headers: { "Authorization": token } })
          .then((response) => {
        ...
        async function deleteExpense(id){
            const token= localStorage.getItem('token');
            try{
                const response= await axios.delete(`http://16.171.60.25:3000/expense/deleteexpense/${id}`,{headers:{'Authorization':token}});
                //console.log(response);
        ...
        document.getElementById('rzp-button1').onclick = async function (e) {
            const token= localStorage.getItem('token');
            //console.log(token);
            const response = await axios.post('http://16.171.60.25:3000/purchase/premiummembership', {}, { headers: { 'Authorization': token } });
            //console.log(response);
            ...
            var options = {
                ...
                "handler": async function (response) {
                        const result=await axios.post('http://16.171.60.25:3000/purchase/updatetransactionstatus', {
            ...
            rzp1.on('payment.failed', async function (response) {
              //console.log(response);
              await axios.post('http://16.171.60.25:3000/purchase/updatetransactionstatusFail', {
                        order_id: options.order_id,
                    }, { headers: {"Authorization" : token} })
...
async function showPremiumFeatures(){
    const token = localStorage.getItem('token');
    const response = await axios.get(`http://16.171.60.25:3000/premium/showleaderboard`, { headers: {"Authorization" : token}});
    //console.log(response);
...
async function download(){
    try{
        const token = localStorage.getItem('token');
        const response= await axios.get('http://16.171.60.25:3000/expense/download',{ headers: {"Authorization" : token} });
...
async function downloadhistory(){
    try{
     const token=localStorage.getItem('token')
     const response= await axios.get('http://16.171.60.25:3000/expense/downloadhistory', {headers: {'Authorization': token}})
